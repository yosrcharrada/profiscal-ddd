...
/// <summary>
/// HTTP wrapper around the EY Azure OpenAI endpoint (GPT-4o only).
///
/// CRITICAL: this endpoint REJECTS requests containing "max_tokens" in the body
/// with HTTP 400. Only send "messages" + "temperature". Do not add max_tokens
/// back even though the interface accepts a maxTokens parameter — that
/// parameter is intentionally ignored in the request body (kept in the
/// signature for interface compatibility / potential future use only).
/// </summary>
public sealed class LlmAgent(IConfiguration config, ILogger<LlmAgent> logger) : ILlmAgent
{
        ...
        CancellationToken ct)
    {
        ...
        // Strip stray quotes/spaces from api version (.env has
        // OPENAI_API_VERSION= "2024-02-15-preview" with leading space + quotes)
        ...
        // CRITICAL: only messages + temperature. NEVER add max_tokens —
        // the EY Azure endpoint returns HTTP 400 if it's present at all,
