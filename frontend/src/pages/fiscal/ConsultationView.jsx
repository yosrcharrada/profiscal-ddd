                ...
                <div className="space-y-8">
                  {SECTIONS.map((s) => (
                    <section key={s.field}>
                      ...
                      {s.field === "analyses" && output.analysisTable?.length > 0 && (
                        <div className="mt-5 border border-gray-200 rounded-xl overflow-x-auto">
                          <table className="w-full text-[12.5px]">
                            <thead>
                              <tr className="bg-gray-900 text-white">
                                <th className="text-left px-3.5 py-2.5 font-semibold">{t("editor.subject")}</th>
                                <th className="text-left px-3.5 py-2.5 font-semibold">{t("editor.analysis")}</th>
                                <th className="text-left px-3.5 py-2.5 font-semibold">{t("editor.conclusion")}</th>
                              </tr>
                            </thead>
                            <tbody>
                              {output.analysisTable.map((r, j) => (
                                <tr key={j} className={j % 2 ? "bg-gray-50" : "bg-white"}>
                                  <td className="px-3.5 py-2.5 align-top font-semibold text-gray-900">{r.sujet}</td>
                                  <td className="px-3.5 py-2.5 align-top text-gray-700 leading-relaxed">{r.analyse}</td>
                                  <td className="px-3.5 py-2.5 align-top font-bold text-gray-900">{r.conclusion}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
