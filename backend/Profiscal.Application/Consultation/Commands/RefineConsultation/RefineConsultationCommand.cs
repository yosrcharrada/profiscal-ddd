    ...
    : IRequestHandler<RefineConsultationCommand, RefineConsultationResult>
{
        ...
        RefineConsultationCommand cmd, CancellationToken ct)
    {
        ...
        // ── Apply update to the section the user selected (authoritative),
        //    or fall back to detecting it from the conversation. ───────────────
        var sectionName = !string.IsNullOrWhiteSpace(cmd.TargetSection)
            ? cmd.TargetSection!.Trim().ToLowerInvariant()
            : DetectSection(cmd.UserMessage, reply);
